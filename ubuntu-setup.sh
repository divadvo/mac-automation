...
# Mobile remote development. Tailscale, OpenSSH, and Mosh are installed, but
# inbound access and authentication changes remain explicitly opt-in.
...
# Claude-only metadata notifications.
...
link_dotfiles() {
  ...
  for rel in \
    ...
    local/bin/sync-sshid-keys
  do
    ln -sfn "$df/$rel" "$HOME/.$rel"
  done
  chmod 0755 \
    "$df/claude/pushover-notify.sh" \
    "$df/local/bin/zj" \
    "$df/local/bin/zj-agent-launcher" \
    "$df/local/bin/sync-sshid-keys"
  ok "symlinked shell, Git, SSH, Claude, tmux, and Zellij configuration"
...
configure_pushover() {
  enabled "$ENABLE_PUSHOVER" || return 0
  log "Configuring Claude Pushover notifications"

  local config_dir="$HOME/.config/ai-notify"
  local config_file="$config_dir/pushover.env"
  local supplied_token="$PUSHOVER_APP_TOKEN"
  local supplied_user="$PUSHOVER_USER_KEY"
  local supplied_device="$PUSHOVER_DEVICE"
  local cached_token="" cached_user="" cached_device=""

  if [[ -r "$config_file" ]]; then
    # This file is generated by this script and is readable only by its owner.
  ...
  if [[ -z "$PUSHOVER_APP_TOKEN" || -z "$PUSHOVER_USER_KEY" ]]; then
    if ! tty_available; then
      die "ENABLE_PUSHOVER=1 requires PUSHOVER_APP_TOKEN and PUSHOVER_USER_KEY in a non-interactive run"
    fi
  ...
  [[ "$PUSHOVER_APP_TOKEN" =~ ^[A-Za-z0-9]{30}$ ]] || die "Pushover application token must be 30 alphanumeric characters"
  [[ "$PUSHOVER_USER_KEY" =~ ^[A-Za-z0-9]{30}$ ]] || die "Pushover user key must be 30 alphanumeric characters"
  ...
  ok "protected Pushover credential cache refreshed"
}
...
install_tailscale() {
  ...
  if $SUDO tailscale status --json 2>/dev/null | jq -e '.BackendState == "Running"' >/dev/null; then
    ...
    log "Connecting Tailscale with TAILSCALE_AUTH_KEY"
...
configure_remote_access() {
  enabled "$ENABLE_REMOTE_ACCESS" || return 0
  ...
  if enabled "$SSH_KEY_ONLY_AUTH" && [[ "$TARGET_USER" == "root" ]]; then
    die "key-only remote access requires a non-root target user because PermitRootLogin is disabled"
  fi

  local sync_helper="$TARGET_HOME/.local/bin/sync-sshid-keys"
  [[ -x "$sync_helper" ]] || die "SSH-ID synchronizer is missing: $sync_helper"
  if [[ $EUID -eq 0 && "$TARGET_USER" != "root" ]]; then
    sudo -u "$TARGET_USER" -H "$sync_helper" "$SSHID_HANDLE"
  else
    HOME="$TARGET_HOME" "$sync_helper" "$SSHID_HANDLE"
  fi

  local sshd_bin dropin_dir dropin temp_dir candidate future_config changed=0
  sshd_bin="$(command -v sshd 2>/dev/null || true)"
  [[ -n "$sshd_bin" ]] || sshd_bin=/usr/sbin/sshd
  dropin_dir=/etc/ssh/sshd_config.d
  dropin="$dropin_dir/99-mac-automation-remote-access.conf"
  [[ -n "$sshd_bin" ]] || die "sshd is not installed"
  grep -Eq '^[[:space:]]*Include[[:space:]]+.*/sshd_config\.d/\*\.conf' /etc/ssh/sshd_config \
    || die "/etc/ssh/sshd_config does not include sshd_config.d; refusing an ineffective change"
  ...
  future_config="$temp_dir/sshd_config"
  {
    ...
    printf 'AllowUsers %s\n' "$TARGET_USER"
    if enabled "$SSH_KEY_ONLY_AUTH"; then
      printf '%s\n' \
  ...
  if ! $SUDO "$sshd_bin" -t -f "$future_config"; then
    rm -rf "$temp_dir"
    die "candidate OpenSSH configuration is invalid; the installed configuration was not changed"
  fi

  if ! $SUDO test -f "$dropin" || ! $SUDO cmp -s "$candidate" "$dropin"; then
    if $SUDO test -f "$dropin"; then
      $SUDO cp "$dropin" "$temp_dir/previous.conf"
    fi
  ...
  if ! $SUDO "$sshd_bin" -t; then
    ...
    rm -rf "$temp_dir"
    die "complete OpenSSH validation failed; the previous drop-in was restored and SSH was not reloaded"
  fi

  if (( changed )); then
    $SUDO systemctl reload ssh
    ok "validated OpenSSH configuration installed and reloaded"
  else
    ok "OpenSSH remote-access configuration is unchanged"
  fi
  rm -rf "$temp_dir"
}

configure_tailscale_firewall() {
  enabled "$TAILSCALE_FIREWALL" || return 0
  log "Adding tailnet-only SSH and Mosh firewall allowances"

  local sshd_bin ports port
  sshd_bin="$(command -v sshd 2>/dev/null || true)"
  [[ -n "$sshd_bin" ]] || sshd_bin=/usr/sbin/sshd
  ports="$($SUDO "$sshd_bin" -T 2>/dev/null | awk '$1 == "port" { print $2 }' | sort -nu)"
  [[ -n "$ports" ]] || ports=22
  ...
  if $SUDO ufw status | grep -q '^Status: active'; then
    ok "tailnet-only UFW allowances are active"
  else
    warn "UFW rules were added but UFW remains inactive; it was not enabled automatically"
  fi
  warn "Existing firewall rules were preserved; review them separately for any pre-existing public SSH access"
}
