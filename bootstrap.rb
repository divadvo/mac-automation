...
class MacBootstrap
  ...
  LOCAL_CONFIG_PATH = File.join(REPO_PATH, "roles/divadvo_mac/vars/local.yml").freeze
  ...
  def get_user_config
    with_step("USER CONFIGURATION") do
      if config_set?
        content = File.read(LOCAL_CONFIG_PATH)
  ...
  def update_config
    with_step("CONFIGURATION UPDATE") do
      if File.exist?(LOCAL_CONFIG_PATH)
        content = File.read(LOCAL_CONFIG_PATH)
        if content.include?(@user_email) && content.include?(@user_name)
          log("Configuration current", type: :success)
          return
        end
      end

      # Create local.yml with user-specific values
      local_content = <<~YAML
        # Local configuration - this file is gitignored
        # Generated by bootstrap.rb

        user_email: "#{@user_email}"
        user_name: "#{@user_name}"
      YAML

      File.write(LOCAL_CONFIG_PATH, local_content)
      log("📝 Email: #{@user_email}")
      log("📝 Name: #{@user_name}")
      log("Configuration saved to local.yml", type: :success)
    end
  end
  ...
  def config_set?
    return false unless File.exist?(LOCAL_CONFIG_PATH)
    content = File.read(LOCAL_CONFIG_PATH)
